...
- ❌ **错误示范**：用 `；` 分隔多个并列子句。AI 模型最常见的"AI 味"标志之一是分号密度过高。中文人类作者几乎不用分号，并列内容更习惯用句号断成短句，或直接用逗号串联
  - 错误：`SpaceX 给 xAI 提供资产容器；xAI 给 SpaceX 提供 IPO 故事；两边各取所需。`
  - 错误：`Google 2004 年引入双层股权；Facebook 2012 年同样是双层结构；Snap 2017 年完全无投票权。`
- ✅ **正确做法**：默认用句号 `。` 替代分号。多数情况下并列子句拆成独立短句更自然。少数确实是连续紧密内容时用逗号即可
  - 正确：`SpaceX 给 xAI 提供资产容器。xAI 给 SpaceX 提供 IPO 故事。两边各取所需。`
  - 正确：`Google 2004 年引入双层股权，Facebook 2012 年走的是同一条路，Snap 2017 年走得最极端，IPO 股票完全无投票权。`
- 🔒 **硬性约束**：写完 Grep `；`。正文（非附录原稿引用块）里默认目标为 0 次。极少数情况确实需要分号才能保留，比如严格列表化的法律或学术陈述
- 📍 **背景**：用户反馈"AI 写的有大量分号，实际人写文章几乎没有分号"。一篇文章常常出现几十处分号，是典型 AI tell
